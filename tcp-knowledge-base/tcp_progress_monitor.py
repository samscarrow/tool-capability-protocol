...
class TCPProgressMonitor:
    def __init__(self, data_dir="data", output_dir="progress_reports"):
        ...
        self.dangerous_commands = {
            "dd", "fdisk", "mkfs", "parted", "shred", "wipefs", "rm", "sudo", 
            "chmod", "chown", "kill", "systemctl", "iptables", "nmap", 
            "tcpdump", "wireshark", "aircrack-ng", "hydra", "john"
        }
        
        # Safe commands for accuracy tracking
        self.safe_commands = {
            "ls", "cat", "echo", "pwd", "date", "whoami", "id", "uptime",
            "which", "whereis", "man", "help", "info", "head", "tail"
        }
                ...
                try:
                    with open(filepath, 'r') as f:
                        ...
                        command_data[command] = {
                            ...
                            'risk': self._extract_risk_from_analysis(data.get('analysis', ''))
                        }
    ...
    def calculate_metrics(self) -> Tuple[ProgressMetrics, QualityMetrics]:
        ...
        if elapsed_hours > 0:
            progress.processing_rate = len(analyzed_commands) / (elapsed_hours * 60)  # per minute
        ...
        quality.man_page_coverage = (man_page_count / len(analyzed_commands) * 100) if analyzed_commands else 0
        
        # Calculate accuracy rate
        total_known = len([c for c in analyzed_commands if c in self.dangerous_commands or c in self.safe_commands])
        if total_known > 0:
            progress.accuracy_rate = ((dangerous_correct + safe_correct) / total_known) * 100
    ...
    def generate_progress_report(self) -> Dict:
        ...
        report = {
            ...
            "progress": {
                ...
                "remaining_commands": progress.total_commands - progress.analyzed_commands,
                "processing_rate_per_minute": progress.processing_rate,
                "estimated_completion_hours": progress.eta_hours,
                "estimated_completion_time": (datetime.now() + timedelta(hours=progress.eta_hours)).isoformat() if progress.eta_hours < 100 else "TBD"
            },
            "quality": {
            ...
            "performance": {
                "average_processing_time_seconds": 60 / progress.processing_rate if progress.processing_rate > 0 else 0,
    ...
    def display_live_dashboard(self):
        ...
        print(f"Analyzed: {progress['analyzed_commands']:,} / {progress['total_commands']:,} commands")
        ...
        print("\n⚡ PERFORMANCE METRICS")
        print("-" * 30)
        print(f"Processing Rate: {progress['processing_rate_per_minute']:.1f} commands/minute")
        print(f"Hourly Capacity: {performance['commands_per_hour']:.0f} commands/hour")
        print(f"Avg Time/Command: {performance['average_processing_time_seconds']:.1f} seconds")
...
"""
        ...
        consortium_file = os.path.join(self.output_dir, f"consortium_report_{datetime.now().strftime('%Y%m%d_%H%M')}.md")
    ...
    def quick_status(self):
        ...
        print(f"🔬 TCP Progress: {progress.analyzed_commands:,}/{progress.total_commands:,} ({(progress.analyzed_commands/progress.total_commands)*100:.1f}%)")
        print(f"⚡ Rate: {progress.processing_rate:.1f}/min | ETA: {progress.eta_hours:.1f}h")
        print(f"🎯 Accuracy: {progress.accuracy_rate:.1f}% | Overrides: {quality.rule_overrides}")

def main():
    import argparse
    
    parser = argparse.ArgumentParser(description="TCP Progress Monitor for Consortium Researchers")
