...
class LocalLLMTCPProcessor:
    ...
    def get_sample_commands(self, count: int = 100) -> List[str]:
        ...
        all_commands = [
            ...
            "head", "tail", "less", "more", "grep", "find", "locate", "which", "file",
            "stat", "du", "df", "ln", "readlink", "basename", "dirname",
            
            # Text processing
            "sed", "awk", "cut", "sort", "uniq", "tr", "wc", "split", "join", "paste",
            "comm", "diff", "patch", "col", "expand", "unexpand", "fmt", "fold",
            
            # System info
            "ps", "top", "htop", "free", "uptime", "who", "w", "last", "id", "groups",
            "hostname", "uname", "date", "cal", "env", "printenv", "alias", "history",
            
            # Network
            "ping", "traceroute", "netstat", "ss", "ip", "ifconfig", "route", "arp",
            "dig", "nslookup", "host", "curl", "wget", "nc", "telnet", "ssh", "scp",
            
            # Process control
            "kill", "killall", "pkill", "nice", "renice", "nohup", "jobs", "fg", "bg",
            "wait", "sleep", "watch", "timeout", "at", "cron", "crontab",
            
            # Permissions & Security
            "chmod", "chown", "chgrp", "umask", "su", "sudo", "passwd", "useradd",
            "userdel", "usermod", "groupadd", "groupdel", "visudo", "chroot",
            
            # Archive & Compression
            "tar", "gzip", "gunzip", "zip", "unzip", "bzip2", "bunzip2", "xz", "7z",
            "rar", "unrar", "zcat", "bzcat", "xzcat",
            
            # Development
            "gcc", "g++", "make", "git", "svn", "diff", "patch", "gdb", "strace",
            "ltrace", "ldd", "nm", "objdump", "strings", "strip",
            
            # Package management
            "apt", "apt-get", "dpkg", "snap", "brew", "port", "pip", "npm", "gem",
            
            # Dangerous commands
            "dd", "mkfs", "fdisk", "parted", "shred", "wipefs"
        ]
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for level in ["CRITICAL", "HIGH_RISK", "MEDIUM_RISK", "LOW_RISK", "SAFE"]:
                    if level.replace("_", " ") in analysis_upper or level in analysis_upper:
                        risk = level
                        break
    ...
    def process_commands_batch(self, commands: List[str], model: str = "phi3:mini"):
        ...
        for i, command in enumerate(commands, 1):
            ...
            if i % 10 == 0:
                ...
                print(f"Progress: {i}/{len(commands)} ({i/len(commands)*100:.1f}%) | "
        ...
        print(f"Average LLM processing: {total_processing_time/len(commands):.0f}ms per command")
    ...
    def save_results(self, filename: str = "local_llm_tcp_results.json"):
        ...
        output = {
            "metadata": {
                ...
                "models_used": list(set(r["model"] for r in self.analysis_results.values())),
