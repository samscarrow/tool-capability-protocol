...
class TCPManPageRefinery:
    ...
    def extract_key_sections(self, man_page: str) -> Dict[str, str]:
        ...
        for line in lines:
            ...
            if line_upper in ["SYNOPSIS", "DESCRIPTION", "OPTIONS", "FLAGS", 
                             "WARNINGS", "CAUTION", "BUGS", "SEE ALSO"]:
    ...
    def analyze_with_ollama(self, command: str, man_sections: Dict[str, str], 
                           model: str = "tinyllama:latest") -> Dict:
...
Be precise and base your assessment on the man page content. Keep under 150 words."""
    ...
    def extract_risk_level(self, analysis: str) -> str:
        ...
        # Infer from content
        danger_words = ["destroy", "delete", "remove", "format", "wipe", "erase", "kill"]
        if any(word in analysis_upper for word in danger_words):
            return "HIGH_RISK"
        
        return "MEDIUM_RISK"  # Default
    
    def extract_capabilities(self, analysis: str, man_sections: Dict[str, str]) -> List[str]:
        ...
        # File operations
        if any(word in combined_text for word in ["file", "directory", "filesystem", "read", "write"]):
            capabilities.append("FILE_OPS")
        
        # Network operations
        if any(word in combined_text for word in ["network", "socket", "port", "tcp", "udp", "http"]):
            capabilities.append("NETWORK")
        
        # Process control
        if any(word in combined_text for word in ["process", "pid", "signal", "kill", "terminate"]):
            capabilities.append("PROCESS")
        
        # System modification
        if any(word in combined_text for word in ["system", "kernel", "module", "service", "daemon"]):
            capabilities.append("SYSTEM")
        
        # Destructive operations
        if any(word in combined_text for word in ["delete", "remove", "destroy", "format", "wipe"]):
            capabilities.append("DESTRUCTIVE")
        
        # Requires privileges
        if any(word in combined_text for word in ["root", "sudo", "privilege", "permission"]):
    ...
    def create_refined_descriptor(self, command: str, analysis: Dict) -> bytes:
        ...
        security_flags = struct.pack('>I', risk_flags.get(analysis["risk"], 0) | cap_flags)
    ...
    def refine_command(self, command: str, existing_analysis: Optional[Dict] = None) -> Dict:
        ...
        if refined_analysis:
            ...
            print(f"   ✅ Refined: [{refined_analysis['risk']}] {', '.join(refined_analysis['capabilities'])}")
    ...
    def demo(self):
        ...
        sample_commands = [
            ...
            # Network tools
            "ping", "curl", "ssh", "nc", "wget",
            # Dangerous commands
            "dd", "mkfs", "fdisk", "shred", "sudo"
        ]
