...
class TCPBinaryDatabaseGenerator:
    ...
    def load_all_analyses(self):
        ...
        analysis_files = [
            f for f in os.listdir(self.data_dir) if f.endswith("_analysis.json")
        ]
    ...
    def create_binary_descriptor(self, command: str, analysis: str) -> bytes:
        ...
        if any(
            ...
            for word in ["file", "directory", "filesystem", "path"]
        ):
        ...
        if any(
            word in analysis_lower
            for word in ["network", "port", "connection", "internet", "download"]
        ):
        ...
        if any(
            word in analysis_lower
            for word in ["root", "sudo", "privilege", "administrator"]
        ):
        ...
        if any(
            word in analysis_lower
            for word in ["destroy", "delete", "remove", "erase", "wipe"]
        ):
            cap_flags |= 0x0800  # DESTRUCTIVE
            self.capability_distribution["DESTRUCTIVE"] += 1

        if any(
            word in analysis_lower for word in ["system", "kernel", "boot", "service"]
        ):
        ...
        if any(
            word in analysis_lower
            for word in ["process", "kill", "terminate", "signal"]
        ):
    ...
    def save_json_export(self):
        ...
        for command, data in sorted(self.descriptors.items()):
            ...
            export_data["commands"][command] = {
                ...
                "analysis_snippet": data["analysis"][:100] + "..."
    ...
    def run(self):
        ...
        print(f"   Commands: {len(self.descriptors):,}")
        print(
            f"   Binary DB: {os.path.getsize(os.path.join(self.output_dir, 'tcp_complete.db')):,} bytes"
        )
        print(
            f"   Compression: ~{len(self.descriptors) * 24:,} bytes for all descriptors"
        )
