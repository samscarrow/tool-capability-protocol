...
Provide logical reasoning and any corrections needed. Keep under 150 words."""
    ...
    def stage4_encoding_review(
        self, command: str, all_previous_data: Dict[str, Any]
    ) -> StageResult:
        ...
        if "parsing" in all_previous_data:
            capabilities_found.update(
                all_previous_data["parsing"].get("operations", [])
            )
        if "logic" in all_previous_data:
            capabilities_found.update(
                all_previous_data["logic"].get("true_capabilities", [])
            )
...
List each flag that should be set and why. Be precise."""
    ...
    def _extract_security_concerns(self, text: str) -> List[str]:
        ...
        if "security" in text_lower or "vulnerability" in text_lower:
            ...
            for s in sentences:
                if any(
                    kw in s.lower() for kw in ["security", "vulnerability", "exploit"]
                ):
    ...
    def demo(self):
        ...
        if "dd" in self.analyses:
            ...
            print(
                f"   Final Risk: {dd_analysis.final_risk.value if dd_analysis.final_risk else 'UNKNOWN'}"
            )
            print(f"   Capabilities: {', '.join(dd_analysis.final_capabilities)}")
            if "safety" in dd_analysis.stages:
                print(
                    f"   Safety Analysis: {dd_analysis.stages['safety'].data.get('safety_analysis', '')[:150]}..."
                )
