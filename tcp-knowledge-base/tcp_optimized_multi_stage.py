...
class OptimizedMultiStageTCPRefinery:
    ...
    def ollama_request(self, prompt: str, model: str, temperature: float = 0.1, 
...
Provide risk level and explain WHY. Be conservative with security."""
    ...
    def _extract_risk_level_enhanced(self, text: str) -> str:
        ...
        critical_keywords = ["destroy", "overwrite", "format", "wipe", "erase", "delete permanently"]
    ...
    def _check_destructive_enhanced(self, text: str) -> bool:
        ...
        destructive_patterns = [
            ...
            "overwrite", "truncate", "shred", "zero out", "permanently"
        ]
    ...
    def _check_privileges_enhanced(self, text: str) -> bool:
        ...
        privilege_patterns = [
            "root", "sudo", "administrator", "privilege", "elevated", 
            "superuser", "admin", "requires root", "must be root"
        ]
        text_lower = text.lower()
...
List each applicable flag with justification."""
    ...
    def _extract_capability_flags(self, response: str, all_data: Dict[str, Any]) -> List[str]:
        ...
        if "sudo" in response_lower or "privilege" in response_lower:
            flags.append("SUDO")
        if "destructive" in response_lower or "destroy" in response_lower or "delete" in response_lower:
    ...
    def process_command(self, command: str) -> CommandAnalysis:
        ...
        if not stage2.success:
            return analysis
        print(f"      ✅ Risk: {stage2.data.get('risk_level')} ({'OVERRIDE' if stage2.data.get('rule_override') else 'LLM'})")
    ...
    def demo(self):
        ...
        test_commands = [
            "dd", "rm", "sudo",    # Should be HIGH_RISK/CRITICAL
            "ls", "cat", "echo"    # Should be SAFE/LOW_RISK
        ]
        ...
        for cmd, analysis in self.analyses.items():
            if analysis.final_risk:
                override = "🔧" if analysis.stages.get("safety", {}).data.get("rule_override") else "🤖"
