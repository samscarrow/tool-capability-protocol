    ...
    def draw_interface(self, stdscr):
        ...
        while True:
            try:
                ...
                row += 1
                stdscr.addstr(row, 4, f"Analyzed: {current_total} ", curses.color_pair(1))
                stdscr.addstr(f"({(current_total/system_total*100):.1f}%)" if system_total > 0 else "")
                row += 1
                ...
                # Discovery rate
                row += 2
                avg_rate = sum(self.discovery_rate) / len(self.discovery_rate) if self.discovery_rate else 0
                stdscr.addstr(row, 2, "⚡ Discovery Rate", curses.A_BOLD)
                row += 1
                stdscr.addstr(row, 4, f"Current: {self.discovery_rate[-1] if self.discovery_rate else 0} cmd/s")
                row += 1
                ...
                stdscr.addstr(height-3, 2, f"Error: {str(e)[:width-4]}", curses.color_pair(3))
                stdscr.refresh()
                time.sleep(2)
