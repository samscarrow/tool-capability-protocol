...
class TCPAwareAgent:
    ...
    def load_tcp_knowledge(self):
        ...
        if result.returncode == 0:
                    ...
                    if result2.returncode == 0:
                        try:
                            ...
                            self.tcp_descriptors[cmd_name] = {
                                ...
                                "requires_sudo": "sudo" in analysis.lower()
                                or "root" in analysis.lower(),
                                "analysis": analysis,
                            }
            ...
            print(
                f"   • Risky tools: {sum(len(cmds) for cmds in self.risky_commands.values())}"
            )
    ...
    def extract_capabilities(self, analysis: str, cmd_name: str) -> List[str]:
        ...
        if any(
            ...
            for word in ["network", "internet", "connection", "port"]
        ):
        ...
        if any(
            word in analysis_lower
            for word in ["security", "scan", "audit", "vulnerability"]
        ):
    ...
    def find_tools_for_task(self, task_description: str) -> Dict[str, List[str]]:
        ...
        relevant_tools = {"recommended": [], "alternative": [], "avoid": []}
        ...
        if any(
            word in task_lower for word in ["network", "download", "upload", "connect"]
        ):
    ...
    def explain_tool(self, command: str) -> str:
        ...
        if command in self.tcp_descriptors:
            ...
            explanation += (
                f"   Requires Sudo: {'Yes' if desc['requires_sudo'] else 'No'}\n"
            )
    ...
    def demonstrate_agent_awareness(self):
        ...
        for cmd in ["ls", "rm", "docker", "nmap", "cat"][:5]:
            if cmd in self.tcp_descriptors:
                ...
                print(
                    f"   {cmd:<10} 0x{risk_byte:02X}  0x{caps_byte:02X}  0x{sudo_byte:02X}  24B"
                )
