...
class EnhancedTCPChainAnalyzer:
    ...
    def analyze_command_chain(self, command_string):
        ...
        safe_alternative = self.generate_safe_alternative(
            commands, operators, risk_level
        )
...
def demonstrate_enhanced_analysis():
    ...
    test_scenarios = [
        {"name": "Simple Commands", "commands": ["ls -la", 'echo "hello"', "rm -rf /"]},
        {
            "name": "AND Chaining (&&)",
            "commands": [
                'echo "start" && ls -la',
    ...
    for scenario in test_scenarios:
        ...
        for command in scenario["commands"]:
            ...
            print(f"\n🔍 {command}")
            print(
                f"   {risk_emoji} Risk: {result['risk_level']} (score: {result['risk_score']:.1f})"
            )
            print(
                f"   🔗 Chain: {result['chain_length']} commands, {result['operator_count']} operators"
            )
            ...
            cmd_risks = [
                f"{cmd['base_command']}({cmd['risk_level']})"
