...
def test_secrets():
    ...
    try:
        # Test Anthropic key
        result = subprocess.run(['op', 'item', 'get', 'ln3svnpksfk7l5zgvylp5ttqki', '--fields', 'credential'], 
                              capture_output=True, text=True)
        if result.returncode == 0:
            anthropic_key = result.stdout.strip()
            print(f"✅ Anthropic API key: {anthropic_key[:20]}... ({len(anthropic_key)} chars)")
        else:
            print(f"❌ Failed to get Anthropic key: {result.stderr}")
            return False
        
        # Test DigitalOcean token
        result = subprocess.run(['op', 'item', 'get', 'izkzdsruj2v6fdya5tzg7mo2sa', '--fields', 'credential'],
                              capture_output=True, text=True)
        if result.returncode == 0:
...
def test_anthropic_api():
    ...
    try:
        ...
        response = client.messages.create(
            ...
            messages=[{"role": "user", "content": "Respond with just 'API_TEST_SUCCESS'"}],
            max_tokens=10
        )
...
def main():
    ...
    if all_tests_passed:
        ...
        print(f"   • Cost approximately ${config['estimated_costs']['total_monthly']}/month")
    else:
