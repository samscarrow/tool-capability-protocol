...
class TCPLiveMonitor:
        ...
        while True:
            try:
                    ...
                    for cmd in sorted(new_commands)[:10]:  # Show first 10
                        ...
                        if analysis:
                            ...
                            self.command_history[cmd].append(
                                {"time": datetime.now().isoformat(), "risk": risk}
                            )
                ...
                if system_count > 0:
                    ...
                    sys.stdout.write(
                        f"\r📈 Progress: [{bar}] {current_count}/{system_count} ({progress:.1f}%)"
                    )
                    sys.stdout.flush()
                ...
                if current_count != last_count:
                    ...
                    if rate > 0:
                        ...
                        sys.stdout.write(
                            f" | Rate: {rate:.1f} cmd/s | ETA: {eta_minutes:.1f}m"
                        )
                        sys.stdout.flush()
    ...
    def show_summary(self):
        ...
        if self.command_history:
            ...
            critical = [
                cmd
                for cmd, h in self.command_history.items()
