...
class CommandChainMathematics:
    ...
    def parse_command_chain(self, command_string):
        ...
        for part in parts:
            ...
            if part in ["&&", "||", "|", ";", "&"]:
                ...
                risk_score = self.command_risks.get(
                    base_cmd, 5
                )  # Default moderate risk
                commands.append({"command": part, "base": base_cmd, "risk": risk_score})
    ...
    def method_1_simple_addition(self, commands, operators):
        ...
        print(
            f"   Command risks: {[cmd['risk'] for cmd in commands]} = {sum(cmd['risk'] for cmd in commands)}"
        )
        print(
            f"   Operator penalty: {len(operators)} operators × 1 = {operator_penalty}"
        )
            ...
            for i, op in enumerate(operators):
                if i + 1 < len(probabilities):
                    ...
                    if op == "&&":
                        ...
                        combined_prob = (
                            combined_prob + next_prob - (combined_prob * next_prob)
                        )
                        print(
                            f"   OR operation: {combined_prob:.3f} (additive with overlap)"
                        )
                    else:  # |, ;, &
                        combined_prob = min(1.0, combined_prob + next_prob * 0.5)
                        print(
                            f"   {op} operation: {combined_prob:.3f} (partial combination)"
                        )
    ...
    def method_5_logarithmic(self, commands, operators):
        ...
        if not base_risks:
            ...
            # Sum of log risks
            log_sum = sum(
                math.log(risk + 1) for risk in base_risks
            )  # +1 to handle 0 risk
            ...
            print(
                f"   Final risk: e^({log_sum:.2f} × {operator_factor:.2f}) - 1 = {final_risk:.2f}"
            )
    ...
    def method_6_tcp_hierarchical(self, commands, operators):
        ...
        for family, family_commands in families.items():
            ...
            print(
                f"   {family} family: {len(family_commands)} commands, max risk {max_risk}, factor {count_factor:.2f} = {family_risk:.2f}"
            )
        ...
        print(
            f"   Final risk: {total_family_risk:.2f} × {operator_complexity:.2f} = {final_risk:.2f}"
        )
...
def demonstrate_mathematical_approaches():
    ...
    test_scenarios = [
        {"name": "Simple Safe Chain", "command": 'echo "start" && ls -la'},
        {"name": "Mixed Risk Chain", "command": "ls -la && cp file.txt backup.txt"},
        {"name": "High Risk Chain", "command": 'find / -name "*.conf" | xargs rm -f'},
        {
