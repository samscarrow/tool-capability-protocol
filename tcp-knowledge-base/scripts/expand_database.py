...
async def expand_database():
    ...
    for pattern, description in test_patterns:
        ...
        if base_cmd in server.ingested_commands:
            ...
            print(
                f'✅ {base_cmd}: {info["risk_level"]} ({info["compression_ratio"]:.0f}:1)'
            )
    ...
    estimated_total_commands = (
        len(server.ingested_commands) * 10
    )  # Estimate if this was 10%
    estimated_total_docs = (
        sum(cmd.get("original_size", 0) for cmd in server.ingested_commands.values())
        * 10
    )
    ...
    print(
        f"   Estimated total docs: {estimated_total_docs:,} bytes ({estimated_total_docs/1024/1024:.1f} MB)"
    )
    print(
        f"   Estimated total TCP: {estimated_total_tcp:,} bytes ({estimated_total_tcp/1024:.1f} KB)"
    )
    print(f"   Estimated compression: {estimated_total_docs/estimated_total_tcp:.0f}:1")
    print(
        f"   Memory efficiency: {estimated_total_tcp/1024/1024:.2f} MB for {estimated_total_commands:,} commands"
    )
