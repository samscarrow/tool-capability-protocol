...
class TCPGroundTruthEnhancer:
    ...
    async def enhance_ground_truth(self, commands_to_analyze: List[str] = None):
        ...
        for i, command in enumerate(commands_to_analyze, 1):
            ...
            improvements = self.compare_analyses(
                command, current_analysis, llm_analysis
            )
...
{{
  ...
  "suggested_patterns": {{"pattern_name": "regex_pattern"}}
        ...
        for command_data in self.enhancement_results["enhanced_commands"].values():
            for improvement in command_data:
                if improvement["type"] == "new_safety_keywords":
                    ...
                    enhanced_capability_patterns[improvement["name"]] = improvement[
                        "pattern"
                    ]
    ...
    def save_enhancement_results(self, output_file: str = "tcp_llm_enhancements.json"):
            ...
            json.dump(
                {
                    "enhancement_metadata": {
                        ...
                        "commands_analyzed": len(
                            self.enhancement_results["enhanced_commands"]
                        ),
                        "total_improvements": sum(
                            len(improvements)
                            for improvements in self.enhancement_results[
                                "enhanced_commands"
                            ].values()
                        ),
                    },
        ...
        print(
            f"   Commands analyzed: {len(self.enhancement_results['enhanced_commands'])}"
        )
        print(
            f"   Improvements found: {sum(len(improvements) for improvements in self.enhancement_results['enhanced_commands'].values())}"
        )
