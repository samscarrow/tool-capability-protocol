...
def main():
    ...
    print(
        f"{'Command':<10} {'Original':<12} {'Enhanced':<12} {'Risk Change':<15} {'Speed':<10}"
    )
    print("-" * 65)
    ...
    for command, test_content in test_cases:
        ...
        original_result = benchmark_analyzer(
            original_analyzer, command, test_content, "original"
        )

        # Benchmark enhanced analyzer
        enhanced_result = benchmark_analyzer(
            enhanced_analyzer, command, test_content, "enhanced"
        )

        # Calculate improvements
        risk_change = (
            f"{original_result['risk_level']} → {enhanced_result['risk_level']}"
        )
        ...
        speed_ratio = (
            original_result["analysis_time_us"] / enhanced_result["analysis_time_us"]
        )
        speed_indicator = (
            f"{speed_ratio:.1f}x" if speed_ratio > 1 else f"1/{1/speed_ratio:.1f}x"
        )

        print(
            f"{command:<10} {original_result['analysis_time_us']:<11.1f}μs {enhanced_result['analysis_time_us']:<11.1f}μs {risk_change:<15} {speed_indicator:<10}"
        )
    ...
    print("-" * 65)
    print(
        f"{'TOTAL':<10} {total_original_time:<11.1f}μs {total_enhanced_time:<11.1f}μs"
    )
    ...
    print(f"\n📈 Enhancement Results:")
    print(
        f"   Accuracy improvements: {accuracy_improvements}/{len(test_cases)} commands ({accuracy_rate*100:.0f}%)"
    )
    print(
        f"   Speed change: {speed_improvement:.2f}x {'faster' if speed_improvement > 1 else 'slower'}"
    )
    ...
    for command, test_content in test_cases[:3]:  # Show first 3 in detail
        ...
        new_caps = set(enhanced.get("capabilities", [])) - set(
    ...
    print(
        f"   • {len(enhanced_analyzer.safety_keywords.get('HIGH_RISK', []))} enhanced keywords"
    )
