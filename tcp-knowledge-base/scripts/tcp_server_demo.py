...
class ImprovedManPageAnalyzer:
    ...
    def __init__(self):
        ...
        self.risk_patterns = {
            ...
            "LOW_RISK": {
                "patterns": [r"read|display|show|list", r"information|status"],
                "commands": ["cat", "less", "grep", "find", "ps", "df"],
            },
            "SAFE": {
                "patterns": [r"display|show|list.*?only", r"read.*?only"],
    ...
    def get_man_page(self, command: str) -> Optional[str]:
        ...
        try:
            ...
            result = subprocess.run(
                ["man", command], capture_output=True, text=True, timeout=5, env=env
            )
...
class TCPServerDemo:
    ...
    async def demonstrate_compression(self):
        ...
        for cmd, desc in commands:
            ...
            if man_content:
                ...
                print(
                    f"| {cmd:<7} | {man_size:>8,} | {tcp_size:>3} | {compression:>10,.0f}:1 | {risk_emoji} {analysis['risk_level']:<10} |"
                )
            else:
    ...
    async def demonstrate_speed(self):
        ...
        for command in test_commands:
            ...
            safe_display = (
                safe_alt[:40] + "..."
                if safe_alt and len(safe_alt) > 40
                else safe_alt or "-"
            )

            print(
                f"| {command:<30} | {decision_time:>6.1f} μs | {risk_emoji} | {safe_display:<40} |"
            )
