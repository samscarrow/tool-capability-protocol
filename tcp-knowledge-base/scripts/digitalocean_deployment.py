    ...
    def analyze_command_batch(self, commands: List[str]) -> Dict:
        ...
        for i, command in enumerate(commands, 1):
            ...
            try:
                ...
                current_analysis = enhanced_analyzer.analyze_man_page(
                    command, man_content
                )
                ...
                if llm_analysis:
                    ...
                    logging.info(
                        f"✅ {command}: {current_analysis['risk_level']} → {llm_analysis.get('risk_level', 'UNKNOWN')}"
                    )
  ...
  "security_notes": "Brief security assessment"
    ...
    def integrate_batch_enhancements(self, batch_results: Dict):
        ...
        for command, data in batch_results.items():
            ...
            for keyword in llm.get("dangerous_keywords", []):
                if (
                    keyword.lower()
                    not in str(current_patterns.get("safety_keywords", {})).lower()
                ):
        ...
        if new_keywords or new_capabilities or risk_corrections:
            ...
            for cap in new_capabilities:
                ...
                enhanced_patterns.setdefault("new_capability_patterns", {})[
                    cap
                ] = pattern
            ...
            self.metrics["patterns_enhanced"] += len(new_keywords) + len(
                new_capabilities
            )
            self.metrics["accuracy_improvements"] += len(risk_corrections)

            logging.info(
                f"✅ Integrated enhancements: {len(new_keywords)} keywords, {len(new_capabilities)} capabilities"
            )
    ...
    def run_enhancement_cycle(self):
        ...
        try:
            ...
            if batch_results:
                ...
                logging.info(
                    f"✅ Enhancement cycle complete: {len(batch_results)} commands processed"
                )
            else:
    ...
    def generate_status_report(self) -> Dict:
        ...
        return {
            ...
            "storage_usage": {
                "patterns_file_size": self.current_patterns.stat().st_size
                if self.current_patterns.exists()
                else 0,
                "history_file_size": self.enhancement_history.stat().st_size
                if self.enhancement_history.exists()
                else 0,
            },
        }
    ...
    def start_continuous_growth(self):
        ...
        schedule.every().hour.do(
            lambda: logging.info(f"📊 Status: {self.generate_status_report()}")
        )
