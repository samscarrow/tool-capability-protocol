...
class TCPDescriptorGenerator:
    ...
    def generate_tcp_descriptor(self, capabilities: ToolCapabilities) -> CapabilityDescriptor:
        ...
        descriptor.input_formats = self._convert_formats(capabilities.input_formats or [])
        descriptor.output_formats = self._convert_formats(capabilities.output_formats or [])
    ...
    def generate_binary_descriptor(self, capabilities: ToolCapabilities) -> bytes:
        ...
        magic = hashlib.md5(capabilities.tool_name.encode()).digest()[:4]
            ...
            for option in cmd.options or []:
                ...
                if param_name:
                    schema["commands"][cmd.name]["parameters"][param_name] = {
                        ...
                        "flags": {
                            "short": option.short_flag,
                            "long": option.long_flag
                        },
                    ...
                    if option.enum_values:
                        schema["commands"][cmd.name]["parameters"][param_name]["enum"] = option.enum_values
                    if option.min_value is not None:
                        schema["commands"][cmd.name]["parameters"][param_name]["minimum"] = option.min_value
                    if option.max_value is not None:
                        schema["commands"][cmd.name]["parameters"][param_name]["maximum"] = option.max_value
                    if option.pattern:
                        schema["commands"][cmd.name]["parameters"][param_name]["pattern"] = option.pattern
    ...
    def _determine_processing_modes(self, capabilities: ToolCapabilities) -> List[ProcessingMode]:
    ...
    def _convert_command(self, enhanced_cmd: EnhancedCommand) -> CommandDescriptor:
        ...
        tcp_cmd = CommandDescriptor(
            name=enhanced_cmd.name,
            description=enhanced_cmd.description,
            parameters=[]
        )
        ...
        for option in enhanced_cmd.options or []:
            param = ParameterDescriptor(
                ...
                type=self.parameter_type_map.get(option.parameter_type, ParameterType.STRING),
                description=option.description,
                required=option.is_required,
                default=option.default_value,
            )
    ...
    def _generate_performance_metrics(self, capabilities: ToolCapabilities) -> PerformanceMetrics:
    ...
    def _generate_capability_flags(self, capabilities: ToolCapabilities) -> Dict[str, bool]:
