...
class TCPGenerationPipeline:
    ...
    def __init__(self, 
        ...
        self.llm_extractor = self._create_llm_extractor(llm_provider, model, **llm_kwargs)
        
    def _create_llm_extractor(self, provider: str, model: str, **kwargs) -> Optional[LLMExtractor]:
    ...
    def process_command(self, 
        ...
        try:
            ...
            if analysis.confidence_score < 0.3:
                self.logger.warning(f"Low confidence in help text parsing: {analysis.confidence_score}")
            ...
            if self.llm_extractor and self.llm_extractor.is_available():
                try:
                    ...
                    if capabilities.confidence_score < 0.5:
                        self.logger.warning(f"Low confidence in LLM extraction: {capabilities.confidence_score}")
            ...
            for output_format in output_formats:
                if output_format == "tcp":
                    ...
                    results["outputs"]["json"] = self.tcp_generator.generate_json_schema(capabilities)
                elif output_format == "binary":
                    binary_data = self.tcp_generator.generate_binary_descriptor(capabilities)
                    results["outputs"]["binary"] = {
                        "data": binary_data.hex(),
                        "size_bytes": len(binary_data)
                    }
                elif output_format == "openapi":
                    results["outputs"]["openapi"] = self._generate_openapi_spec(capabilities)
                else:
    ...
    def batch_process_commands(self, 
                              commands: List[str],
        ...
        for command in commands:
            ...
            try:
                ...
                if result["success"]:
                    ...
                    if output_path:
                        cmd_filename = command.replace(" ", "_").replace("/", "_") + ".json"
    ...
    def _basic_capability_extraction(self, analysis: HelpTextAnalysis) -> ToolCapabilities:
        ...
        option_flags = [opt.long_flag or opt.short_flag or "" for opt in analysis.global_options]
        option_text = " ".join(option_flags + [opt.description for opt in analysis.global_options])
        
        # Basic capability inference
        capabilities.supports_stdin = any(
            keyword in option_text.lower() 
            for keyword in ["stdin", "input", "-"]
        )
    ...
    def _calculate_avg_confidence(self, results: Dict[str, Any]) -> float:
        ...
        for result in results.values():
            if result.get("success") and result.get("capabilities"):
                confidence_scores.append(result["capabilities"].get("confidence_score", 0))
