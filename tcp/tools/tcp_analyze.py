...
Available LLM providers:
  ...
  - basic: No LLM, basic parsing only
    ...
    # Input options
    parser.add_argument(
        "--help-text", help="Pre-extracted help text (skip automatic extraction)"
    )
    ...
    parser.add_argument(
        "--output-dir", type=Path, help="Output directory for results (default: stdout)"
    )
    ...
    # Processing options
    parser.add_argument(
        "--batch", action="store_true", help="Process commands in batch mode"
    )
    ...
    try:
        ...
        if args.batch or len(args.commands) > 1:
            ...
            if results.get("capabilities"):
                ...
                if confidence < args.confidence_threshold:
                    logger.warning(
                        f"Low confidence score: {confidence} < {args.confidence_threshold}"
                    )
        ...
        if args.output_file:
            ...
            with open(args.output_file, "w") as f:
                json.dump(
                    output_data, f, indent=2 if args.pretty else None, default=str
                )
    ...
    for provider in ["basic", "local_llm", "langchain_openai"]:
        ...
        try:
            ...
            if result["success"]:
                ...
                print(
                    f"     Supports recursion: {caps.get('supports_recursion', False)}"
                )
