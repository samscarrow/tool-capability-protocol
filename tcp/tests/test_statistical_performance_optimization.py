...
class TestStatisticalValidation:
    ...
    def test_running_statistics_accuracy(self):
        ...
        assert abs(stats.mean - expected_mean) < 1e-10, f"Mean calculation error: {stats.mean} != {expected_mean}"
        assert abs(stats.variance - expected_variance) < 1e-10, f"Variance calculation error: {stats.variance} != {expected_variance}"
        assert abs(stats.std_dev - expected_std) < 1e-10, f"StdDev calculation error: {stats.std_dev} != {expected_std}"
    ...
    def test_confidence_intervals(self):
        ...
        assert ci_95[0] < stats.mean < ci_95[1], "Mean should be within confidence interval"
        
        # Validate approximate correctness for large sample
        expected_margin_95 = 1.96 * stats.standard_error
        assert abs(margin_95 - 2 * expected_margin_95) < 0.5, f"CI margin calculation error"
    ...
    def test_numerical_stability(self):
        ...
        assert not math.isnan(stats.variance), "Variance calculation produced NaN for small values"
    ...
    def test_welford_algorithm_equivalence(self):
        ...
        # Compare results
        assert abs(welford_stats.mean - batch_mean) < 1e-10, "Mean mismatch between algorithms"
        assert abs(welford_stats.variance - batch_var) < 1e-10, "Variance mismatch between algorithms"
        assert abs(welford_stats.std_dev - batch_std) < 1e-10, "StdDev mismatch between algorithms"
...
class TestPerformanceEngine:
    ...
    def test_measurement_recording(self, engine):
        ...
        required_keys = ['operation_id', 'measurement', 'timestamp', 'statistics', 
                        'anomaly_detection', 'baseline_comparison', 'performance']
    ...
    def test_performance_engine_overhead(self, engine):
        ...
        assert avg_time_per_measurement < 0.1, f"Statistical analysis overhead too high: {avg_time_per_measurement:.3f}ms"
...
class TestOptimizedProtocol:
    ...
    def test_optimized_tool_selection(self, optimized_protocol, sample_descriptors):
        ...
        assert second_time < first_time, f"Cache hit not faster: {second_time:.6f}s vs {first_time:.6f}s"
        
        # Validate selection correctness
        assert result1 is not None
        fastest_tool = min(sample_descriptors, key=lambda t: t.performance.avg_processing_time_ms)
    ...
    def test_batch_binary_generation(self, optimized_protocol, sample_descriptors):
        ...
        assert cached_time < 0.01, f"Cached binary generation too slow: {cached_time:.6f}s"
    
    def test_capability_query_optimization(self, optimized_protocol, sample_descriptors):
        ...
        results = optimized_protocol.optimized_capability_query(sample_descriptors, capabilities)
...
class TestMathematicalCorrectness:
    ...
    def test_confidence_interval_coverage(self):
        ...
        assert abs(coverage_rate - expected_rate) < margin, \
