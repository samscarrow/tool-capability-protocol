...
class TCPSandboxManager:
    ...
    def request_tool_approval(
        ...
        request_file = (
            self.sandbox_dir / "approval_requests" / f"{tool_name}_approval.json"
        )
    ...
    def approve_tool(
        ...
        final_permission = permission_level or SandboxPermission(
            request_data["requested_permission"]
        )
        ...
        self.logger.info(
            f"Tool approved by {approved_by}: {tool_name} with {final_permission.value} permission"
        )
    ...
    def get_available_tools(self) -> Dict[str, SandboxedTool]:
        ...
        return {
            name: tool
            for name, tool in self.approved_tools.items()
    ...
    def validate_tool_execution(
        ...
        if tool.allowed_args:
            for arg in args:
                allowed = any(
                    allowed_pattern in arg for allowed_pattern in tool.allowed_args
                )
    ...
    def execute_sandboxed_tool(
        self, tool_name: str, args: List[str], input_data: str = None, timeout: int = 30
    ) -> Dict:
        """
        ...
        with tempfile.TemporaryDirectory(
            dir=self.sandbox_dir / "temp_workspace"
        ) as temp_dir:
            try:
                # Prepare command
                ...
                self.logger.info(
                    f"Tool execution completed: {tool_name} (exit code: {result.returncode})"
                )
    ...
    def revoke_tool_approval(
        self, tool_name: str, revoked_by: str, reason: str
    ) -> bool:
        ...
        self.logger.warning(
            f"Tool approval revoked: {tool_name} by {revoked_by} - {reason}"
        )
    ...
    def get_security_status(self) -> Dict:
        ...
        return {
            ...
            "pending_approvals": len(
                list((self.sandbox_dir / "approval_requests").glob("*.json"))
            ),
            "quarantined_tools": len(
                list((self.sandbox_dir / "quarantine").glob("*.json"))
            ),
