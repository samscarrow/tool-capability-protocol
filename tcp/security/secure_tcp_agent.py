...
class SecureTCPAgent:
    ...
    def _refresh_capability_cache(self) -> None:
        ...
        for tool_name, tool in approved_tools.items():
            ...
            try:
                ...
                self.logger.warning(f"Failed to parse capabilities for {tool_name}: {e}")
        
        self.logger.info(f"Capability cache refreshed: {len(self.capability_cache)} tools available")
    ...
    def request_tool_execution(self,
                             tool_name: str,
                             args: List[str],
                             input_data: str = None,
    ...
    def suggest_tool_for_task(self, task_description: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'confidence': matches / len(capability_requirements) if capability_requirements else 0.5,
            'capabilities': capabilities,
    ...
    def generate_capability_report(self) -> str:
        ...
        for tool_name, tool_info in self.capability_cache.items():
            ...
            if tool_info['forbidden_args']:
                report_lines.append(f"   Forbidden args: {', '.join(tool_info['forbidden_args'])}")
    ...
    def _show_task_suggestion(self, task: str, suggestion: Dict) -> None:
        ...
        if suggestion['suggestion']:
            ...
            print(f"   Required capabilities: {', '.join(suggestion.get('required_capabilities', []))}")
            print(f"   Available tools: {', '.join(suggestion.get('available_tools', []))}")
    ...
    def _handle_exec_command(self, command: str) -> None:
        ...
        result = self.request_tool_execution(tool_name, args, justification=justification)
