...
class OllamaLLMProcessor:
    ...
    def analyze_command_security(self, command: str, man_page_content: str) -> Dict[str, Any]:
...
"""
        ...
        try:
            ...
            with urlopen(req, timeout=60) as response:
                ...
                try:
                    ...
                    required_fields = ["security_level", "privilege_requirements", "risk_score"]
    ...
    def _fallback_analysis(self, command: str, man_page_content: str) -> Dict[str, Any]:
        ...
        if command in critical_commands:
            ...
            risk_score = 0.5
        elif any(word in content_lower for word in ['delete', 'remove', 'destroy', 'format']):
            security_level = "high_risk"
            risk_score = 0.8
        else:
            security_level = "safe"
        ...
        if any(word in content_lower for word in ['root', 'superuser', 'administrator']):
...
class LocalTCPSecurityDemo:
    ...
    def __init__(self, demo_dir: str = None):
        ...
        self.sandbox = TCPSandboxManager(str(self.demo_dir / "sandbox"), security_level="strict")
    ...
    def demonstrate_local_workflow(self) -> None:
        ...
        for command in test_commands:
            ...
            base_data.destructive_operations = llm_analysis.get("destructive_operations", [])
            ...
            print(f"   ✅ Binary: {len(binary_data)} bytes, flags: 0x{descriptor.security_flags:08x}")
    ...
    def demonstrate_secure_agent_with_local_analysis(self, processed_data: Dict) -> None:
        ...
        for command in safe_tools:
            ...
            if approved:
                print(f"   ✅ {command} approved (local analysis: {data['llm_analysis']['security_level']})")
        ...
        dangerous_result = agent.request_tool_execution(
            "rm",
            ["-rf", "*"],
            justification="Testing dangerous command blocking"
        )
        
        if not dangerous_result['success']:
            print(f"   ✅ Dangerous command correctly blocked: {dangerous_result.get('reason')}")
        else:
    ...
    def generate_local_demo_report(self, processed_data: Dict) -> str:
        ...
        llm_analyzed = sum(1 for data in processed_data.values() 
