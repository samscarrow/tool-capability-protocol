...
from .manpage_enricher import ManPageEnricher, ManPageData, SecurityLevel, PrivilegeLevel
...
class EnrichedTCPEncoder:
    ...
    def encode_enhanced_tcp(self, command: str) -> EnrichedTCPDescriptor:
        ...
        magic_signature = hashlib.md5(command.encode()).digest()[:4]
    ...
    def _build_operation_flags(self, man_data: ManPageData) -> int:
        ...
        content = (man_data.description + " " + 
                  " ".join(opt.get('description', '') for opt in man_data.options)).lower()
        ...
        # File operations
        if any(word in man_data.file_operations for word in ['read', 'write', 'modify']):
    ...
    def _build_security_flags(self, man_data: ManPageData) -> int:
        ...
        if man_data.destructive_operations:
            ...
            irreversible_keywords = ['permanent', 'cannot be undone', 'irreversible', 'destroy']
            if any(keyword in " ".join(man_data.destructive_operations).lower() 
        ...
        if any(word in " ".join(man_data.system_operations).lower() 
    ...
    def from_binary(self, binary_data: bytes) -> EnrichedTCPDescriptor:
        ...
        if len(binary_data) != 24:
            raise ValueError(f"Invalid binary data length: {len(binary_data)} (expected 24)")
    ...
    def batch_encode_commands(self, commands: List[str]) -> Dict[str, EnrichedTCPDescriptor]:
        ...
        for i, command in enumerate(commands, 1):
            ...
            try:
                ...
                print(f"      {emoji} {descriptor.security_level.value} | {descriptor.privilege_requirements.value}")
...
def main():
    ...
    test_commands = [
        ...
        'cp', 'mv', 'curl', 'wget', 'tar',
        # High risk  
        'chmod', 'chown', 'kill', 'mount', 'ssh',
        # Critical
        'rm', 'dd', 'sudo', 'fdisk', 'mkfs'
    ]
    ...
    critical_commands = [cmd for cmd, desc in encoded.items() 
