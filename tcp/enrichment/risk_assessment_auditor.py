...
class TransparentRiskAssessor:
    ...
    def _init_risk_databases(self) -> None:
        ...
        self.privilege_indicators = {
            'root_required': [
                'must be run as root', 'requires root privileges', 'superuser only',
                'run as root', 'root access required', 'administrative privileges'
            ],
            'sudo_suggested': [
                'requires sudo', 'may require sudo', 'elevated privileges',
                'administrative rights', 'superuser privileges'
            ],
            'system_level': [
                'system configuration', 'system files', 'kernel modules',
                'device access', 'hardware control', 'system services'
            ]
        }
        
        # Destructive operation keywords
        self.destructive_keywords = {
            'permanent_loss': ['destroy', 'delete permanently', 'irreversible', 'cannot be undone'],
            'data_modification': ['overwrite', 'replace', 'modify', 'change'],
            'system_impact': ['format', 'wipe', 'erase', 'clear', 'purge'],
            'security_impact': ['remove permissions', 'change ownership', 'bypass security']
        }
        ...
        self.network_risk_factors = {
            ...
            'security_implications': ['authentication', 'encryption', 'certificate', 'key']
        }
    
    def assess_command_risk(self, command: str, man_data: ManPageData) -> SecurityClassificationAudit:
        """
    ...
    def _analyze_command_name_risk(self, command: str, audit: SecurityClassificationAudit) -> None:
    ...
    def _analyze_privilege_requirements(self, man_data: ManPageData, audit: SecurityClassificationAudit) -> None:
    ...
    def _analyze_destructive_capabilities(self, man_data: ManPageData, audit: SecurityClassificationAudit) -> None:
    ...
    def _analyze_network_operations(self, man_data: ManPageData, audit: SecurityClassificationAudit) -> None:
        ...
        for operation in man_data.network_operations:
            ...
            if any(keyword in operation.lower() for keyword in ['upload', 'send', 'transmit']):
                risk_score = 0.6
    ...
    def _analyze_file_operations(self, man_data: ManPageData, audit: SecurityClassificationAudit) -> None:
    ...
    def _analyze_system_operations(self, man_data: ManPageData, audit: SecurityClassificationAudit) -> None:
    ...
    def _analyze_security_notes(self, man_data: ManPageData, audit: SecurityClassificationAudit) -> None:
        ...
        for note in man_data.security_notes:
            ...
            if any(keyword in note_lower for keyword in ['dangerous', 'caution', 'warning']):
                risk_score = 0.8
            elif any(keyword in note_lower for keyword in ['careful', 'security', 'risk']):
                risk_score = 0.6
    ...
    def _analyze_manpage_keywords(self, man_data: ManPageData, audit: SecurityClassificationAudit) -> None:
        """Analyze risk-indicating keywords in man page content."""
        
        high_risk_keywords = [
            'destroy', 'delete', 'remove', 'erase', 'wipe', 'format',
            'overwrite', 'replace', 'modify', 'change', 'alter'
        ]
    ...
    def _analyze_command_options(self, man_data: ManPageData, audit: SecurityClassificationAudit) -> None:
    ...
    def _analyze_usage_examples(self, man_data: ManPageData, audit: SecurityClassificationAudit) -> None:
        ...
        for example in man_data.examples:
            ...
            if any(pattern in example_lower for pattern in ['/dev/', 'rm -rf', 'dd if=', 'format']):
                evidence = RiskEvidence(
            ...
            elif any(pattern in example_lower for pattern in ['/etc/', '/var/', '/usr/', '/sys/']):
                evidence = RiskEvidence(
                    factor_type=RiskFactor.EXAMPLE_ANALYSIS,
    ...
    def _calculate_final_scores(self, audit: SecurityClassificationAudit) -> None:
        ...
        privilege_evidence = [e for e in audit.risk_evidence 
        ...
        destructive_evidence = [e for e in audit.risk_evidence 
                              if e.factor_type == RiskFactor.DESTRUCTIVE_CAPABILITY]
        
        if destructive_evidence:
            audit.destructive_score = max(e.risk_contribution for e in destructive_evidence)
        else:
    ...
    def _make_security_classification(self, audit: SecurityClassificationAudit) -> None:
        ...
        if score >= 0.9 or audit.destructive_score >= 0.9:
            ...
            audit.security_rationale = f"Low risk (score: {score:.2f}) with minimal security implications"
        else:
    ...
    def _make_privilege_classification(self, audit: SecurityClassificationAudit) -> None:
        ...
        if score >= 0.8:
            ...
            audit.privilege_rationale = f"Requires elevated privileges (score: {score:.2f}) for safe operation"
        else:
            audit.final_privilege_level = PrivilegeLevel.USER
            audit.privilege_rationale = f"User-level privileges sufficient (score: {score:.2f})"
    
    def _generate_classification_summary(self, audit: SecurityClassificationAudit) -> None:
        """Generate human-readable classification summary."""
        
        evidence_count = len(audit.risk_evidence)
        high_risk_evidence = len([e for e in audit.risk_evidence if e.risk_contribution >= 0.7])
    ...
    def save_audit_report(self, audit: SecurityClassificationAudit) -> str:
        ...
        audit_filename = f"{audit.command}_risk_audit_{audit.classification_timestamp[:10]}.json"
        ...
        audit_data['audit_metadata'] = {
            ...
            'high_risk_evidence_count': len([e for e in audit.risk_evidence if e.risk_contribution >= 0.7]),
            'evidence_sources': list(set(audit.data_sources)),
            'classifier_confidence': sum(e.confidence for e in audit.risk_evidence) / len(audit.risk_evidence)
        }
    ...
    def generate_human_readable_report(self, audit: SecurityClassificationAudit) -> str:
        ...
        for factor_type, evidence_list in evidence_by_type.items():
            ...
            for evidence in sorted(evidence_list, key=lambda x: x.risk_contribution, reverse=True):
                risk_level = "HIGH" if evidence.risk_contribution >= 0.7 else "MED" if evidence.risk_contribution >= 0.4 else "LOW"
                report_lines.append(f"  [{risk_level}] {evidence.evidence_text}")
                report_lines.append(f"       Risk: {evidence.risk_contribution:.2f}, Confidence: {evidence.confidence:.2f}")
...
def main():
    ...
    if man_data:
        ...
        print("-" * 50)
        print(readable_report[:1500] + "..." if len(readable_report) > 1500 else readable_report)
