...
from enrichment.risk_assessment_auditor import TransparentRiskAssessor, SecurityClassificationAudit
            ...
            if permission != SandboxPermission.DENIED:
                ...
                try:
                    approved = self.sandbox.request_tool_approval(
                        ...
                        allowed_args=["--help", "-h"] if permission == SandboxPermission.READ_ONLY else None,
                        forbidden_args=["-f", "--force", "-r", "-R"] if audit.destructive_score > 0.5 else None
                    )
    ...
    def demonstrate_security_enforcement(self) -> None:
        ...
        if approved_tools:
            ...
            if result['success']:
                ...
                print(f"❌ {test_tool} execution failed: {result.get('reason', 'Unknown')}")
        else:
        ...
        if approved_tools:
            ...
            if test_tool:
                ...
                if result['success']:
                    ...
                    print(f"✅ Forbidden argument blocked: {result.get('reason', 'Unknown')}")
            else:
    ...
    def save_demonstration_artifacts(self, processed_commands: Dict) -> None:
        ...
        for command, data in processed_commands.items():
            if data.get('audit'):
                ...
                readable_report = self.risk_assessor.generate_human_readable_report(data['audit'])
        ...
        for command, data in processed_commands.items():
            if data.get('descriptor'):
                ...
                descriptors_data[command] = {
                    ...
                    'security_level': desc.security_level.value if hasattr(desc.security_level, 'value') else str(desc.security_level),
                    'privilege_requirements': desc.privilege_requirements.value if hasattr(desc.privilege_requirements, 'value') else str(desc.privilege_requirements),
