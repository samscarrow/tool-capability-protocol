...
class DiscoveryService:
    ...
    def find_best_tool(
        self, task_requirements: Dict[str, Any], optimization_target: str = "speed"
        ...
        candidates = self.discover(formats=formats, commands=commands)
        ...
        for candidate in candidates:
            ...
            if (
                ...
                and perf.concurrent_requests < concurrent_requests
            ):
                continue
    ...
    def discover_similar_tools(
        self, reference_tool: CapabilityDescriptor, similarity_threshold: float = 0.7
    ) -> List[CapabilityDescriptor]:
    ...
    def _apply_filters(
        self, filter_criteria: DiscoveryFilter
    ) -> List[CapabilityDescriptor]:
    ...
    def _matches_filter(
        self, tool: CapabilityDescriptor, filter_criteria: DiscoveryFilter
    ) -> bool:
        ...
        if filter_criteria.performance_criteria:
            if not self._meets_performance_criteria(
                tool, filter_criteria.performance_criteria
            ):
                return False
    ...
    def _meets_performance_criteria(
        self, tool: CapabilityDescriptor, criteria: Dict[str, Any]
    ) -> bool:
    ...
    def _select_optimal(
        self, tools: List[CapabilityDescriptor], target: str
    ...
    def _calculate_similarity(
        self, tool1: CapabilityDescriptor, tool2: CapabilityDescriptor
    ) -> float:
        ...
        if perf1.avg_processing_time_ms > 0 and perf2.avg_processing_time_ms > 0:
            time_ratio = min(
                perf1.avg_processing_time_ms, perf2.avg_processing_time_ms
            ) / max(perf1.avg_processing_time_ms, perf2.avg_processing_time_ms)
            perf_score += time_ratio
            perf_count += 1

        # Compare memory usage
        if perf1.memory_usage_mb > 0 and perf2.memory_usage_mb > 0:
            memory_ratio = min(perf1.memory_usage_mb, perf2.memory_usage_mb) / max(
                perf1.memory_usage_mb, perf2.memory_usage_mb
            )
