...
class SemanticRoutingGraph:
    ...
    def __init__(self):
        ...
        self.path_performance_history: Dict[str, deque] = defaultdict(lambda: deque(maxlen=100))
        self.node_load_history: Dict[str, deque] = defaultdict(lambda: deque(maxlen=100))
    ...
    def add_semantic_node(self, node_id: str, 
        ...
        logger.debug(f"Added semantic node {node_id} with {len(capabilities)} capabilities")
    
    def add_semantic_edge(self, source: str, destination: str,
                         latency_ms: float,
                         bandwidth_mbps: float,
                         reliability: float,
                         security_level: float) -> None:
    ...
    def update_node_behavioral_state(self, node_id: str,
                                   trust_score: float,
                                   anomaly_score: float,
        ...
        if node_id in self.behavioral_baselines:
            for metric, value in behavioral_metrics.items():
                if metric in self.behavioral_baselines[node_id]:
                    ...
                    self.behavioral_baselines[node_id][metric] = current * 0.9 + value * 0.1
        
        logger.debug(f"Updated behavioral state for {node_id}: trust={trust_score:.3f}, anomaly={anomaly_score:.3f}")
    
    def get_content_affinity_score(self, node_id: str, content_type: ContentType) -> float:
    ...
    def calculate_path_semantic_score(self, path: List[str], context: RoutingContext) -> float:
        ...
        for node_id in path:
            ...
            # Content affinity
            content_affinity = self.get_content_affinity_score(node_id, context.content_type)
        ...
        for i in range(len(path) - 1):
            ...
            if edge_key in self.edge_characteristics:
                ...
                if context.max_latency_ms:
                    ...
                    edge_score += max(0.0, 1.0 - latency / 100.0) * 0.4  # Assume 100ms baseline
    ...
    def _get_current_node_load(self, node_id: str) -> float:
        ...
        if node_id in self.node_load_history and self.node_load_history[node_id]:
            ...
            recent_loads = list(self.node_load_history[node_id])[-10:]  # Last 10 measurements
...
class SemanticRoutingEngine:
    ...
    def __init__(self):
        ...
        self.route_performance: Dict[str, deque] = defaultdict(lambda: deque(maxlen=1000))
        self.path_success_rates: Dict[str, float] = defaultdict(lambda: 1.0)
        
        # Adaptive learning
        self.content_routing_patterns: Dict[ContentType, Dict[str, float]] = defaultdict(dict)
    ...
    def add_network_node(self, node_id: str,
                        capabilities: Set[str],
    ...
    async def find_semantic_path(self, source: str, destination: str,
                               context: RoutingContext,
                               strategy: str = "default") -> Optional[SemanticPath]:
        ...
        if strategy in self.routing_strategies:
            path_candidates = await self.routing_strategies[strategy](source, destination, context)
        else:
            path_candidates = await self._default_routing_strategy(source, destination, context)
    ...
    async def find_multi_path_routes(self, source: str, destination: str,
                                   context: RoutingContext,
                                   num_paths: int = 3) -> List[SemanticPath]:
        ...
        for i in range(num_paths):
            ...
            if excluded_nodes:
                ...
                modified_context.destination_constraints['excluded_nodes'].update(excluded_nodes)
            ...
            if path:
                ...
                intermediate_nodes = set(path.nodes[1:-1])  # Exclude source and destination
    ...
    async def update_behavioral_state(self, node_id: str,
                                    trust_score: float,
                                    anomaly_score: float,
                                    behavioral_metrics: Dict[str, float]) -> None:
        ...
        await self._learn_from_behavioral_update(node_id, anomaly_score, behavioral_metrics)
    
    async def adaptive_path_optimization(self, path_id: str,
                                       performance_feedback: Dict[str, float]) -> Optional[SemanticPath]:
        ...
        actual_latency = performance_feedback.get('actual_latency_ms', target_path.total_latency_ms)
        ...
        if actual_latency > target_path.total_latency_ms * 1.5 or success_rate < 0.8:
            ...
            optimization_context = RoutingContext(
                ...
                performance_requirements={'max_latency_ms': target_path.total_latency_ms * 0.8},
    ...
    async def _default_routing_strategy(self, source: str, destination: str,
                                      context: RoutingContext) -> List[List[str]]:
    ...
    async def _behavioral_aware_strategy(self, source: str, destination: str,
                                       context: RoutingContext) -> List[List[str]]:
        ...
        if source in safe_nodes and destination in safe_nodes:
            ...
            try:
                ...
                for path in nx.shortest_simple_paths(safe_subgraph, source, destination):
                    paths.append(path)
    ...
    def _calculate_content_affinities(self, capabilities: Set[str]) -> Dict[ContentType, float]:
        ...
        behavioral_match = len(capabilities & behavioral_capabilities) / len(behavioral_capabilities)
        affinities[ContentType.BEHAVIORAL_DATA] = behavioral_match
        
        # Security alert processing
        security_capabilities = {'encrypt', 'decrypt', 'audit', 'verify'}
        security_match = len(capabilities & security_capabilities) / len(security_capabilities)
        affinities[ContentType.SECURITY_ALERT] = security_match
        
        # Performance metrics processing
        performance_capabilities = {'measure', 'benchmark', 'profile', 'optimize'}
        performance_match = len(capabilities & performance_capabilities) / len(performance_capabilities)
        ...
        topology_match = len(capabilities & topology_capabilities) / len(topology_capabilities)
        affinities[ContentType.NETWORK_TOPOLOGY] = topology_match
        
        return affinities
    
    def _calculate_semantic_edge_weight(self, source: str, destination: str,
        ...
        if context.security_requirements:
            required_security = context.security_requirements.get('min_security_level', 0.5)
    ...
    def _select_optimal_semantic_path(self, path_candidates: List[List[str]],
                                    context: RoutingContext) -> Optional[SemanticPath]:
        ...
        for path_nodes in path_candidates:
            ...
            content_affinity = self.routing_graph.calculate_path_semantic_score(path_nodes, context)
            
            # Create semantic path object
            path_id = hashlib.md5('->'.join(path_nodes).encode()).hexdigest()[:8]
            ...
            semantic_path = SemanticPath(
                ...
                behavioral_safety_score=self._calculate_behavioral_safety(path_nodes, context),
        ...
        best_path = max(scored_paths, key=lambda p: self._calculate_overall_path_score(p, context))
        
        return best_path
    
    def _calculate_overall_path_score(self, path: SemanticPath, context: RoutingContext) -> float:
    ...
    def _calculate_path_latency(self, path_nodes: List[str]) -> float:
        ...
        for i in range(len(path_nodes) - 1):
            ...
            if edge_key in self.routing_graph.edge_characteristics:
                total_latency += self.routing_graph.edge_characteristics[edge_key]['latency_ms']
            else:
                total_latency += 50.0  # Default latency for unknown edges
        
        return total_latency
    
    def _calculate_behavioral_safety(self, path_nodes: List[str], context: RoutingContext) -> float:
...
async def demonstrate_semantic_routing():
    ...
    nodes = [
        ("tcp-validator-1", {"validate", "transform", "compress"}, {"tcp_validation_latency": 0.24}),
        ("tcp-validator-2", {"validate", "optimize", "cache"}, {"tcp_validation_latency": 0.18}),
        ("behavioral-analyzer", {"analyze", "detect", "monitor", "aggregate"}, {"anomaly_detection_rate": 0.02}),
        ("security-node", {"encrypt", "decrypt", "audit", "verify"}, {"security_response_time": 8.0}),
        ("performance-monitor", {"measure", "benchmark", "profile"}, {"monitoring_latency": 5.0}),
        ("load-balancer", {"route", "balance", "distribute"}, {"routing_accuracy": 0.98})
    ]
    ...
    for node_id, capabilities, behavioral_profile in nodes:
        ...
        routing_engine.add_network_node(node_id, capabilities, behavioral_profile, trust_score)
        print(f"   {node_id}: {len(capabilities)} capabilities, trust={trust_score:.2f}")
    ...
    for content_type, source, destination in content_scenarios:
        ...
        # Find optimal path
        optimal_path = await routing_engine.find_semantic_path(source, destination, context)
    ...
    print(f"   Features: Content-aware routing, behavioral integration, adaptive optimization")
