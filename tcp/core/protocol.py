...
class ToolCapabilityProtocol:
    ...
    def get_tool(self, name: str, version: Optional[str] = None) -> Optional[CapabilityDescriptor]:
    ...
    def validate_descriptor(self, descriptor: CapabilityDescriptor) -> List[str]:
        ...
        for command in descriptor.commands:
            ...
            for param in command.parameters:
                if not param.name:
                    errors.append(f"Parameter name is required in command {command.name}")
                elif param.name in param_names:
                    errors.append(f"Duplicate parameter name: {param.name} in command {command.name}")
                else:
    ...
    def _binary_to_descriptor(self, binary_desc: BinaryCapabilityDescriptor) -> CapabilityDescriptor:
