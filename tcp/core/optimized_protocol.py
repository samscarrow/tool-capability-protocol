...
from .descriptors import (
    CapabilityDescriptor, BinaryCapabilityDescriptor, CommandDescriptor,
    ParameterDescriptor, FormatDescriptor, PerformanceMetrics, CapabilityFlags
)
...
from ..analysis.statistical_performance_engine import (
    StatisticalPerformanceEngine, OptimizationBackend, PerformanceStatistics
)
...
class OptimizedToolCapabilityProtocol(ToolCapabilityProtocol):
    ...
    def __init__(self, 
        ...
        if enable_optimization:
            ...
            logger.info(f"Optimized TCP protocol initialized with backend: {optimization_backend}")
        else:
    ...
    def select_optimal_tool(self,
                           ...
                           criteria: str = "speed") -> Optional[CapabilityDescriptor]:
        """
        ...
        cache_key = f"optimal_tool:{criteria}:{hashlib.md5(':'.join(tool_fingerprints).encode()).hexdigest()[:16]}"
        ...
        if 'baseline_comparison' in result and result['baseline_comparison']['effect_size']:
            effect_size = result['baseline_comparison']['effect_size']
            if effect_size > 0.5:  # Medium effect size
                logger.debug(f"Tool selection optimization showing {effect_size:.2f} effect size improvement")
        
        return optimal_tool
    
    def _optimized_tool_selection(self, 
                                 tools: List[CapabilityDescriptor], 
                                 criteria: str) -> Optional[CapabilityDescriptor]:
        """
    ...
    def batch_validate_descriptors(self, 
                                  descriptors: List[CapabilityDescriptor]) -> List[Tuple[CapabilityDescriptor, List[str]]]:
        """
        ...
        self.stats_engine.record_measurement(
            ...
            metadata={
                ...
                'avg_validation_time_ms': sum(validation_times) / len(validation_times) if validation_times else 0,
    ...
    def batch_generate_binary(self, 
                             descriptors: List[CapabilityDescriptor]) -> List[bytes]:
        """
        ...
        self.stats_engine.record_measurement(
            ...
            metadata={
                ...
                'avg_binary_time_ms': sum(binary_times) / len(binary_times) if binary_times else 0
            }
        )
        
        return results
    
    def optimized_capability_query(self, 
                                  tools: List[CapabilityDescriptor],
                                  capability_queries: List[str]) -> Dict[str, Dict[str, bool]]:
        """
        ...
        if not self.enable_optimization:
            ...
            for tool in tools:
                ...
                for capability in capability_queries:
                    tool_results[capability] = self.query_capabilities(tool.name, capability, tool.version)
        ...
        for tool in tools:
            ...
            queries_hash = hashlib.md5(':'.join(sorted(capability_queries)).encode()).hexdigest()[:16]
            ...
            if cached_capabilities is not None:
                ...
                for capability in capability_queries:
                    tool_results[capability] = self._optimized_capability_check(tool, capability)
    ...
    def _optimized_capability_check(self, tool: CapabilityDescriptor, capability: str) -> bool:
        """
    ...
    def get_optimization_report(self) -> Dict[str, Any]:
        ...
        for op_id, stats in self.stats_engine.running_stats.items():
            if stats.count > 10:  # Only report operations with sufficient data
                ...
                if baseline:
                    improvement_factor = baseline.mean / stats.mean if stats.mean > 0 else 1.0
                    operation_improvements[op_id] = {
                        "baseline_mean_ms": baseline.mean,
                        "optimized_mean_ms": stats.mean,
                        "improvement_factor": improvement_factor,
                        "confidence_interval": stats.confidence_interval(self.statistical_confidence),
    ...
    def establish_performance_baselines(self, 
        ...
        for operation_id, values in baseline_data.items():
            if values:
                baseline_stats = self.stats_engine.establish_baseline(operation_id, values)
                self._operation_baselines[operation_id] = baseline_stats
                logger.info(f"Baseline established for {operation_id}: {baseline_stats.mean:.2f}ms ± {baseline_stats.std_dev:.2f}ms")
