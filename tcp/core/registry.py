...
class CapabilityRegistry:
    ...
    def register(self, descriptor: CapabilityDescriptor) -> bool:
        ...
        with self._lock:
            try:
                ...
                self._access_times[(descriptor.name, descriptor.version)] = datetime.utcnow()
    ...
    def get(self, name: str, version: Optional[str] = None) -> Optional[CapabilityDescriptor]:
    ...
    def find_by_capability(self, capability: str, value: Optional[str] = None) -> List[CapabilityDescriptor]:
    ...
    def find_by_format(self, format_name: str, is_input: bool = True) -> List[CapabilityDescriptor]:
        """Find tools that support a specific format."""
        with self._lock:
            results = []
            
            for descriptor in self._get_all_tools():
                formats = descriptor.input_formats if is_input else descriptor.output_formats
    ...
    def get_statistics(self) -> Dict[str, int]:
        ...
        with self._lock:
            ...
            return {
                ...
                'total_formats': sum(
                    len(d.input_formats) + len(d.output_formats) 
                    for d in all_tools
                ),
    ...
    def _has_capability(self, descriptor: CapabilityDescriptor, capability: str, value: Optional[str]) -> bool:
