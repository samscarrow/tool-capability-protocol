            ...
            for line in lines:
                if line.strip():
                    ...
                    if match:
                        ...
                        if not any(skip in cmd.lower() for skip in ['.', '_', 'lib', 'man', 'conf']):
        ...
        for i, command in enumerate(commands, 1):
            ...
            try:
                ...
                if man_content:
                    ...
                    print(f"{risk_emoji} {risk_level} ({original_size:,} → {tcp_size} bytes = {compression_ratio:.0f}:1)")
    ...
    async def test_adaptive_intelligence(self, ingested_commands):
        ...
        for query in test_queries:
            ...
            if base_cmd in self.server.ingested_commands:
                ...
                print(f"   Flags: {', '.join(info['flags']) if info['flags'] else 'None'}")
            else:
                print(f"   🔍 Not in database - would generate on-demand")
                # Could analyze here but keeping it fast for demo
    
    async def generate_scaling_report(self, results, total_original, total_tcp, risk_dist):
        ...
        best_compression = max(results, key=lambda x: x['compression_ratio']) if results else None
        worst_compression = min(results, key=lambda x: x['compression_ratio']) if results else None
        
        print(f"📈 Ingestion Statistics:")
        print(f"   Commands processed: {successful_ingestions}")
        print(f"   Total documentation: {total_original:,} bytes ({total_original/1024/1024:.1f} MB)")
        print(f"   Total TCP descriptors: {total_tcp:,} bytes ({total_tcp/1024:.1f} KB)")
        print(f"   Overall compression: {average_compression:.1f}:1")
        print(f"   Space savings: {((total_original - total_tcp) / total_original * 100):.1f}%")
        
        if best_compression:
            print(f"\n🏆 Best compression: {best_compression['command']}")
            print(f"   {best_compression['original_size']:,} → {best_compression['tcp_size']} bytes")
            print(f"   Ratio: {best_compression['compression_ratio']:.1f}:1")
        
        if worst_compression:
            print(f"\n📉 Lowest compression: {worst_compression['command']}")
            print(f"   {worst_compression['original_size']:,} → {worst_compression['tcp_size']} bytes") 
        ...
        memory_per_command = total_tcp / successful_ingestions if successful_ingestions > 0 else 0
        print(f"\n💾 Memory Efficiency:")
        ...
        print(f"   Memory for 10,000 commands: {memory_per_command * 10000 / 1024:.1f} KB")
...
async def main():
    ...
    results, total_orig, total_tcp, risk_dist = await tester.ingest_sampled_commands(available_commands)
