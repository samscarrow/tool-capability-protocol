...
class AdvancedTCPSecurity:
    ...
    def verify_security_layers(self, descriptor, signature):
        ...
        expected_signature = hmac.new(self.hmac_key, descriptor, hashlib.sha256).digest()
    ...
    def demonstrate_multi_layer_security(self):
        ...
        for scenario in attack_scenarios:
            ...
            # Execute attack
            tampered_desc, tampered_sig = scenario['attack'](bytearray(descriptor), signature)
    ...
    def benchmark_security_performance(self):
        ...
        descriptor, signature = self.create_signed_tcp_descriptor("rm -rf /", "CRITICAL")
        ...
        print(f"   CRC16 only: {(crc_time/iterations)*1_000_000:.2f} μs per verification")
        print(f"   HMAC only: {(hmac_time/iterations)*1_000_000:.2f} μs per verification")
        print(f"   Combined: {(combined_time/iterations)*1_000_000:.2f} μs per verification")
        
        print(f"\n📊 Security vs Performance Trade-off:")
        print(f"   Basic TCP: ~2-50 μs (no security)")
        print(f"   TCP + CRC: ~{2 + (crc_time/iterations)*1_000_000:.1f}-{50 + (crc_time/iterations)*1_000_000:.1f} μs (integrity)")
        print(f"   TCP + HMAC: ~{2 + (hmac_time/iterations)*1_000_000:.1f}-{50 + (hmac_time/iterations)*1_000_000:.1f} μs (authentication)")
        print(f"   TCP + Both: ~{2 + (combined_time/iterations)*1_000_000:.1f}-{50 + (combined_time/iterations)*1_000_000:.1f} μs (maximum security)")
