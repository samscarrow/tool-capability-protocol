...
async def quick_sample_test():
    ...
    try:
        result = subprocess.run(['man', '-k', '.'], capture_output=True, text=True, timeout=10)
        if result.returncode == 0:
            ...
            print(f"Sample: {', '.join(sampled[:10])}{'...' if len(sampled) > 10 else ''}")
            ...
            for i, cmd in enumerate(sampled, 1):
                ...
                try:
                    ...
                    if man_content:
                        ...
                        print(f"{risk_emoji} {analysis['risk_level']} ({compression:.0f}:1)")
                        results.append((cmd, analysis['risk_level'], compression, orig_size))
                    else:
                        print("❌ No man page")
