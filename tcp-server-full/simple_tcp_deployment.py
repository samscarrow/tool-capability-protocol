...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('tcp_knowledge_growth.log'),
    ...
    def __init__(self):
        ...
        try:
            result = subprocess.run(
                ['op', 'item', 'get', 'neposatk4gkawjw52mlhm6jvcu', '--fields', 'credential'],
    ...
    def load_state(self):
        ...
        if state_file.exists():
            try:
                ...
                logging.info(f"📊 Loaded state: {len(self.analyzed_commands)} analyzed commands")
            ...
            if result.returncode == 0:
                ...
                for line in result.stdout.split('\n')[:500]:  # Limit scope
                    if line.strip():
                        ...
                        if len(cmd) > 1 and cmd.isalnum() and cmd not in self.analyzed_commands:
                            all_commands.append(cmd)
                
                # Return new commands up to limit
                new_commands = list(set(all_commands))[:limit]
                logging.info(f"🔍 Discovered {len(new_commands)} new commands to analyze")
                return new_commands
...
Respond with JSON only."""
            ...
            logging.info(f"✅ {command}: {current['risk_level']} → {llm_analysis.get('risk_level', 'UNKNOWN')}")
            return result
    ...
    def run_enhancement_cycle(self):
        ...
        if results:
            ...
            results_file = self.data_dir / f"enhancement_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            ...
            for result in results:
                ...
                if current_risk != llm_risk:
                    logging.info(f"🔍 Risk disagreement on {result['command']}: {current_risk} vs {llm_risk}")
    ...
    def get_stats(self):
        ...
        return {
            ...
            'last_cycle': self.enhancement_history[-1]['timestamp'] if self.enhancement_history else None,
